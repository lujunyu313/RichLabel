...
var RichLabel = cc.Layer.extend({
	...
	stopAnim: function() {
		...
		for(var i in this._spriteArray) {
	...
	/**
	 *	播放动画FadeIn
	 *	@param{Number} wordPerSec：每秒播放多少个字，默认为15个
	 *	@param{Callback} cb
	 */
	...
	_parseString: function(str) {
		...
		// 删除</>
		if(tagHead) {
			for(var i = 0; i < tagHead.length; i++) {
				if(tagHead[i].match(/<\//)) {
					tagHead.splice(i, 1);
				}
			}
		...
		for(var i in parseArray) {
			(function(i){
				...
				case "text":	//文本
					...
					var fontName = parse.fontName || self._fontName;
					var fontSize = parse.fontSize || self._fontSize;
					var fontColor = parse.fontColor || self._fontColor;
					
					for(var j in textArray) {
						var label = new cc.LabelTTF(textArray[j], fontName, fontSize);
						attr.anchorY = 0;
							...
							cc.eventManager.addListener({
								...
								onTouchBegan: function(touch, event) {
									...
									if (cc.rectContainsPoint(rect, locationInNode)) {
										...
										for(var j in self._spriteArray) {
											if(index == self._spriteArray[j].index) {
												var children = self._spriteArray[j].getChildren();
												color = children[0].color;
												for(var c in children) {
													children[c].color = cc.color(255, 0, 0);
												}
												//self._spriteArray[j].scale = self._spriteArray[j].scale * 0.8;
											}
										}
										return true;
									}
				...
				case "animation":	//动画
					...
					sprite.attr(attr);
					spriteArray.push(sprite);
					self._containLayer.addChild(spritebatch);
	...
	_getPointOfSprites: function(spriteArray, dimensions) {
		...
		for(var i in rowArray) {
			var rowHeight = 0;
			var space = 0;
			for(var j in rowArray[i]) {		//获取行的最高精灵	
				var index = rowArray[i][j];
				var height = 0;
			...
			var pointY = curY + rowHeight + space;
		...
		for(var i = 0; i < spriteNum;i++) {
			var index = indexYArray[i];
			var pointY = rowHeightArray[index];
